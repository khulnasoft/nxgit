...
import (
	...
	_ "go.khulnasoft.com/nxgit/modules/minwinsvc" // import minwinsvc for windows services
...
var (
	...
	UI = struct {
		...
		Meta: struct {
			...
			Author:      "Nxgit - Git with a cup of tea",
			Description: "Nxgit (Git with a cup of tea) is a painless self-hosted Git service written in Go",
			Keywords:    "go,git,self-hosted,nxgit",
		},
	}
...
func getWorkPath(appPath string) string {
	workPath := ""
	nxgitWorkPath := os.Getenv("NXGIT_WORK_DIR")

	if len(nxgitWorkPath) > 0 {
		workPath = nxgitWorkPath
	} else {
...
func NewContext() {
	...
	CustomPath = os.Getenv("NXGIT_CUSTOM")
	if len(CustomPath) == 0 {
	...
	AppName = Cfg.Section("").Key("APP_NAME").MustString("Nxgit: Git with a cup of tea")
	...
	CookieUserName = sec.Key("COOKIE_USERNAME").MustString("nxgit_awesome")
	CookieRememberName = sec.Key("COOKIE_REMEMBER_NAME").MustString("nxgit_incredible")
