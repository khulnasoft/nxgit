...
func (p *Parser) Render(rawBytes []byte, urlPrefix string, metas map[string]string, isWiki bool) []byte {
	var (
		...
		command = strings.NewReplacer(envMark("NXGIT_PREFIX_SRC"), urlPrefix,
			envMark("NXGIT_PREFIX_RAW"), urlRawPrefix).Replace(p.Command)
	...
	if p.IsInputFile {
		// write to temp file
		f, err := ioutil.TempFile("", "nxgit_input")
		if err != nil {
	...
	cmd.Env = append(
		os.Environ(),
		"NXGIT_PREFIX_SRC="+urlPrefix,
		"NXGIT_PREFIX_RAW="+urlRawPrefix,
	)
	if !p.IsInputFile {
		cmd.Stdin = rd
	}
