...
import (
	...
	"go.khulnasoft.com/nxgit/routers/routes"
...
func initIntegrationTest() {
	nxgitRoot := os.Getenv("NXGIT_ROOT")
	if nxgitRoot == "" {
		fmt.Println("Environment variable $NXGIT_ROOT not set")
		os.Exit(1)
	}
	setting.AppPath = path.Join(nxgitRoot, "nxgit")
	if _, err := os.Stat(setting.AppPath); err != nil {
		fmt.Printf("Could not find nxgit binary at %s\n", setting.AppPath)
		os.Exit(1)
	}

	nxgitConf := os.Getenv("NXGIT_CONF")
	if nxgitConf == "" {
		fmt.Println("Environment variable $NXGIT_CONF not set")
		os.Exit(1)
	} else if !path.IsAbs(nxgitConf) {
		setting.CustomConf = path.Join(nxgitRoot, nxgitConf)
	} else {
		setting.CustomConf = nxgitConf
	}
	...
	case setting.UseMySQL:
		...
		if _, err = db.Exec("CREATE DATABASE IF NOT EXISTS testnxgit"); err != nil {
	...
	case setting.UsePostgreSQL:
		...
		if _, err = db.Exec("CREATE DATABASE testnxgit"); err != nil {
			log.Fatalf("db.Exec: %v", err)
		}
	case setting.UseMSSQL:
		...
		if _, err := db.Exec("If(db_id(N'nxgit') IS NULL) BEGIN CREATE DATABASE nxgit; END;"); err != nil {
			log.Fatalf("db.Exec: %v", err)
		}
		defer db.Close()
	}
...
func prepareTestEnv(t testing.TB) {
	...
	assert.NoError(t, com.CopyDir(path.Join(filepath.Dir(setting.AppPath), "integrations/nxgit-repositories-meta"),
