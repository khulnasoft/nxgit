...
func availableVersions() ([]string, error) {
	...
	versionRE, err := regexp.Compile("nxgit-v(?P<version>.+)\\." + regexp.QuoteMeta(models.DbCfg.Type) + "\\.sql.gz")
	if err != nil {
		return nil, err
	}
...
func readSQLFromFile(version string) (string, error) {
	filename := fmt.Sprintf("integrations/migration-test/nxgit-v%s.%s.sql.gz", version, models.DbCfg.Type)
...
func restoreOldDB(t *testing.T, version string) bool {
	...
	case setting.UseMSSQL:
		...
		_, err = db.Exec("DROP DATABASE IF EXISTS nxgit")
		assert.NoError(t, err)

		_, err = db.Exec("CREATE DATABASE nxgit")
		assert.NoError(t, err)
