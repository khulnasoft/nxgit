...
For issues reaching repositories over `ssh` while the Nxgit web front-end, but
...
* On the client:
  * Ensure the public and private ssh keys are added to the correct Nxgit user.
...
* On the server:
  ...
  * Verify that the correct public keys are added to `.ssh/authorized_keys`.
    Try to run `Rewrite '.ssh/authorized_keys' file (for Nxgit SSH keys)` on the
    Nxgit admin panel.
  * Read Nxgit logs.
...
* On the server:
  ...
  * Ensure that the `nxgit serv` command in `.ssh/authorized_keys` uses the
 ...
 * (Re-)sync tags of all repositories within Nxgit:
 ```
 nxgit admin repo-sync-releases
 ```
...
```
batch response: Authentication required: Authorization error: <NXGIT_LFS_URL>/info/lfs/objects/batch                                                                                                              
