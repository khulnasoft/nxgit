...
`nxgit [global options] command [command options] [arguments...]`

### Global options
 - `--help`, `-h`: Show help text and exit. Optional. This can be used with any of the
   subcommands to see help text for it.
 - `--version`, `-v`: Show version and exit. Optional. (example: `Nxgit version
...
- Options:
    ...
    - `--config path`, `-c path`: Nxgit configuration file path. Optional. (default: custom/conf/app.ini).
    - `--pid path`, `-P path`: Pidfile path. Optional.
- Examples:
    - `nxgit web`
    - `nxgit web --port 80`
    - `nxgit web --config /etc/nxgit.ini --pid /var/run/nxgit.pid`
- Notes:
    - Nxgit should not be run as root. To bind to a port below 1000, you can use setcap on
      Linux: `sudo setcap 'cap_net_bind_service=+ep' /path/to/nxgit`. This will need to be
      redone every time you update Nxgit.
...
- Commands:
    - `create-user`
        - Options:
            ...
            - `--config path`: Nxgit configuration file path. Optional. (default: custom/conf/app.ini).
        ...
        - Examples:
            - `nxgit admin create-user --name myname --password asecurepassword --email me@example.com`
    - `change-password`
        - Options:
            ...
            - `--config path`: Nxgit configuration file path. Optional. (default: custom/conf/app.ini).
        - Examples:
            - `nxgit admin change-password --username myname --password asecurepassword`
    - `regenerate`
        - Options:
            - `hooks`: Regenerate git-hooks for all repositories
            - `keys`: Regenerate authorized_keys file
        - Examples:
            - `nxgit admin regenerate hooks`
            - `nxgit admin regenerate keys`
    - `auth`:
        - `list`:
        ...
        - `add-oauth`:
            ...
            - Examples:
                - `nxgit auth add-oauth --name external-github --provider github --key OBTAIN_FROM_SOURCE --secret OBTAIN_FROM_SOURCE`
        - `update-oauth`:
            - Options:
                ...
                - `--custom-email-url`: Use a custom Email URL (option for GitHub).
            - Examples:
                - `nxgit auth update-oauth --id 1 --name external-github-updated`
...
- Examples:
    - `nxgit cert --host git.example.com,example.com,www.example.com --ca`

#### dump

Dumps all files and databases into a zip file. Outputs into a file like `nxgit-dump-1482906742.zip`
...
- Commands:
    - `secret`:
        ...
        - Examples:
            - `nxgit generate secret INTERNAL_TOKEN`
            - `nxgit generate secret LFS_JWT_SECRET`
            - `nxgit generate secret SECRET_KEY`
...
# username running nxgit
AuthorizedKeysCommandUser git
AuthorizedKeysCommand /path/to/nxgit keys -e git -u %u -t %t -k %k
```
...
NB: opensshd requires the nxgit program to be owned by root and not
