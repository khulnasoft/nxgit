...
import (
	...
	"go.khulnasoft.com/nxgit/modules/avatar"
	"go.khulnasoft.com/nxgit/modules/base"
	"go.khulnasoft.com/nxgit/modules/generate"
...
func synchronizeLdapSSHPublicKeys(usr *User, s *LoginSource, SSHPublicKeys []string) bool {
	...
	for _, v := range keys {
		nxgitKeys = append(nxgitKeys, v.OmitEmail())
	}
	...
	if util.IsEqualSlice(nxgitKeys, ldapKeys) {
		log.Trace("synchronizeLdapSSHPublicKeys[%s]: LDAP Public Keys are already in sync for %s (LDAP:%v/DB:%v)", s.Name, usr.Name, len(ldapKeys), len(nxgitKeys))
		return false
	}
	log.Trace("synchronizeLdapSSHPublicKeys[%s]: LDAP Public Key needs update for user %s (LDAP:%v/DB:%v)", s.Name, usr.Name, len(ldapKeys), len(nxgitKeys))

	// Add LDAP Public SSH Keys that doesn't already exist in DB
	var newLdapSSHKeys []string
	for _, LDAPPublicSSHKey := range ldapKeys {
		if !util.ExistsInSlice(LDAPPublicSSHKey, nxgitKeys) {
	...
	var nxgitKeysToDelete []string
	for _, nxgitKey := range nxgitKeys {
		if !util.ExistsInSlice(nxgitKey, ldapKeys) {
			log.Trace("synchronizeLdapSSHPublicKeys[%s]: Marking LDAP Public SSH Key for deletion for user %s: %v", s.Name, usr.Name, nxgitKey)
			nxgitKeysToDelete = append(nxgitKeysToDelete, nxgitKey)
		}
	}

	// Delete LDAP keys from DB that doesn't exist in LDAP
	needUpd, err := deleteKeysMarkedForDeletion(nxgitKeysToDelete)
	if err != nil {
