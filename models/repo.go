...
func NewRepoContext() {
	...
	if version.Compare("1.7.1", setting.Git.Version, ">") {
		log.Fatal(4, "Nxgit requires Git version greater or equal to 1.7.1")
	}

	// Git requires setting user.name and user.email in order to commit changes.
	for configKey, defaultValue := range map[string]string{"user.name": "Nxgit", "user.email": "nxgit@fake.local"} {
...
func createDelegateHooks(repoPath string) (err error) {
	...
	for i, hookName := range hookNames {
		...
		newHookPath := filepath.Join(hookDir, hookName+".d", "nxgit")
		...
		if err = ioutil.WriteFile(newHookPath, []byte(nxgitHookTpls[i]), 0777); err != nil {
...
func initRepository(e Engine, repoPath string, u *User, repo *Repository, opts CreateRepoOptions) (err error) {
	...
	tmpDir := filepath.Join(os.TempDir(), "nxgit-"+repo.Name+"-"+com.ToStr(time.Now().Nanosecond()))
