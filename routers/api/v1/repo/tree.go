...
func GetTreeBySHA(ctx *context.APIContext, sha string) *nxgit.GitTreeResponse {
	gitTree, err := ctx.Repo.GitRepo.GetTree(sha)
	if err != nil || gitTree == nil {
		return nil
	}
	tree := new(nxgit.GitTreeResponse)
	...
	tree.Entries = make([]nxgit.GitEntry, rangeEnd-rangeStart)
